...
class NotificationOutboxHealthService:
    OUTBOX_WORKER_HEARTBEAT_SOURCE = "notification_outbox_worker"
    ...
    @staticmethod
    def _get_or_create_heartbeat_row(
        db: Session, *, source: str
    ) -> NotificationOutboxWorkerHeartbeat:
        row = (
            db.query(NotificationOutboxWorkerHeartbeat)
            .filter(NotificationOutboxWorkerHeartbeat.source == source)
            .first()
        )
        if row is not None:
            return row

        row = NotificationOutboxWorkerHeartbeat(source=source)
        db.add(row)
        try:
            db.flush()
            return row
    ...
    def record_worker_started(
        db: Session,
        *,
        source: str = OUTBOX_WORKER_HEARTBEAT_SOURCE,
        started_at: Optional[datetime] = None,
    ) -> None:
        started = ensure_utc(started_at) or utcnow()
        row = NotificationOutboxHealthService._get_or_create_heartbeat_row(
            db, source=source
        )
    ...
    def get_pipeline_health_snapshot(
        ...
        *,
        ...
        heartbeat_row = (
            db.query(NotificationOutboxWorkerHeartbeat)
            .filter(
                NotificationOutboxWorkerHeartbeat.source
                == NotificationOutboxHealthService.OUTBOX_WORKER_HEARTBEAT_SOURCE
            )
            .first()
        )
        ...
        if heartbeat_row is not None:
            heartbeat_reference = ensure_utc(
                heartbeat_row.last_completed_at
            ) or ensure_utc(heartbeat_row.last_started_at)

        heartbeat_age_seconds = None
        if heartbeat_reference is not None:
            heartbeat_age_seconds = max(
                (current - heartbeat_reference).total_seconds(),
                0.0,
            )
        ...
        due_pending_count, oldest_due_retry_at = (
            db.query(
                ...
                func.min(NotificationOutbox.next_retry_at),
            )
            .filter(
                NotificationOutbox.status == "pending",
                NotificationOutbox.next_retry_at <= current,
            )
            .one()
        )
        total_pending_count = (
        ...
        stale_lock_cutoff = current - timedelta(
            seconds=settings.NOTIFICATION_OUTBOX_HEALTH_STALE_LOCK_SECONDS
        )
        stale_processing_count = (
            ...
            .filter(
                ...
                NotificationOutbox.locked_at <= stale_lock_cutoff,
            )
            .scalar()
            or 0
        )
        ...
        heartbeat_stale = (
            ...
            > settings.NOTIFICATION_OUTBOX_HEALTH_HEARTBEAT_MAX_AGE_SECONDS
        )
        if heartbeat_stale:
            breaches.append("worker_heartbeat_stale")
        if (
            int(due_pending_count or 0)
            > settings.NOTIFICATION_OUTBOX_HEALTH_MAX_DUE_PENDING_COUNT
        ):
            breaches.append("due_pending_count_exceeded")
        if (
            oldest_due_pending_age_seconds is not None
            and oldest_due_pending_age_seconds
            > settings.NOTIFICATION_OUTBOX_HEALTH_MAX_OLDEST_DUE_PENDING_SECONDS
        ):
            breaches.append("oldest_due_pending_age_exceeded")
        if (
            int(failed_count or 0)
            > settings.NOTIFICATION_OUTBOX_HEALTH_MAX_FAILED_COUNT
        ):
            breaches.append("failed_count_exceeded")
        if (
            int(stale_processing_count or 0)
            > settings.NOTIFICATION_OUTBOX_HEALTH_MAX_STALE_LOCK_COUNT
        ):
        ...
        return {
            ...
            "breaches": breaches,
            "heartbeat": {
                "source": NotificationOutboxHealthService.OUTBOX_WORKER_HEARTBEAT_SOURCE,
                "missing": heartbeat_row is None,
                "stale": heartbeat_stale,
                "age_seconds": heartbeat_age_seconds,
                "last_started_at": (
                    NotificationOutboxHealthService._isoformat_or_none(
            ...
            "queue": {
                ...
                "oldest_due_retry_at": NotificationOutboxHealthService._isoformat_or_none(
                    oldest_due_retry_at_utc
                ),
                "oldest_due_pending_age_seconds": oldest_due_pending_age_seconds,
            },
            "thresholds": {
                "heartbeat_max_age_seconds": settings.NOTIFICATION_OUTBOX_HEALTH_HEARTBEAT_MAX_AGE_SECONDS,
                "max_due_pending_count": settings.NOTIFICATION_OUTBOX_HEALTH_MAX_DUE_PENDING_COUNT,
                "max_oldest_due_pending_seconds": settings.NOTIFICATION_OUTBOX_HEALTH_MAX_OLDEST_DUE_PENDING_SECONDS,
                "max_failed_count": settings.NOTIFICATION_OUTBOX_HEALTH_MAX_FAILED_COUNT,
                "stale_lock_seconds": settings.NOTIFICATION_OUTBOX_HEALTH_STALE_LOCK_SECONDS,
                "max_stale_processing_count": settings.NOTIFICATION_OUTBOX_HEALTH_MAX_STALE_LOCK_COUNT,
            },
        }
