...
const AnalyticsPage = () => {
  ...
  const visibleUserGrowthRangeLabel = useMemo(() => {
    ...
    const firstMonth = visibleUserGrowth[0]?.month;
    const lastMonth = visibleUserGrowth[visibleUserGrowth.length - 1]?.month;
                ...
                <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div className="row">
                    <button
                      ...
                      disabled={loading || userGrowthPage <= 1}
                      onClick={() => setUserGrowthPage((prev) => Math.max(1, prev - 1))}
                    >
                      Previous
                    </button>
                    <button
                      type="button"
                      className="btn btn-secondary"
                      disabled={loading || userGrowthPage >= userGrowthTotalPages}
                      onClick={() =>
                        setUserGrowthPage((prev) => Math.min(userGrowthTotalPages, prev + 1))
                      }
                    >
                      Next
                    </button>
